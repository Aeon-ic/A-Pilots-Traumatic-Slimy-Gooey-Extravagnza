...
public class FireablePistol : MonoBehaviour, IFireable
{
  ...
  public float bulletDistance = 75;
  ...
  private void Awake()
  {
    ...
    bulletRender = this.gameObject.GetComponent<LineRenderer>();
  }
  ...
  public void Shoot()
  {
    ...
    if (ammoLoaded > 0)
    {
      ...
      ammoLoaded -= 1;
      //Check if it hit
      if (Physics.Raycast(this.gameObject.transform.position, this.gameObject.transform.up, out hit, bulletDistance))
      {
  ...
  IEnumerator DrawBullet()
  {
    ...
    if (hit.collider == null)
    {
      ...
      bulletRender.SetPosition(0, this.gameObject.transform.position);
